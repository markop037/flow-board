...
def get_task(db: DBSession, task_id: uuid.UUID) -> TaskRead:
    task = task_service.get_by_id_or_404(db, task_id)
    return task  # type: ignore[return-value]


@router.post("/", response_model=TaskRead, status_code=status.HTTP_201_CREATED)
def create_task(db: DBSession, task_in: TaskCreate) -> TaskRead:
...
def delete_task(db: DBSession, task_id: uuid.UUID) -> None:
    task = task_service.get_by_id_or_404(db, task_id)
